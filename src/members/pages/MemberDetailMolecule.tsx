...
export const MemberDetailMolecule = (props: MemberDetailMoleculeProps) => {
    ...
    return (
        <Vertical className="dl-container">
            <Horizontal>
                <b style={{ fontSize: '1.1em' }}>
                    {props.data.membership?.firstname} {props.data.membership?.lastname} ({props.data.user_nicename})
                </b>
                <Checkbox
                    kind="toggle"
                    className="fg-0"
                    value={editState}
                    onChange={ev => setEditState(ev.target.checked)}
                >
                    Ändern
                </Checkbox>
            </Horizontal>
            ...
            >
                ...
                <DataElement label="Email">
                    ...
                    <a href={'mailto:' + props.data.user_email}>
                        <AiOutlineMail />
                    </a>
                </DataElement>
                <DataElement
                    ...
                    className="fg-0"
                >
                    {(props.data.membership && props.data.membership.member && <b>JA</b>) || 'NEIN'}
                </DataElement>
            </Horizontal>
            {!editState && props.data.membership && props.data.membership.member && (
                <>
                    <Horizontal
                        ...
                        style={{ gap: '0px' }}
                    >
                        <DataElement label="Adresse">
                            {props.data.membership.street}, {props.data.membership.plz} {props.data.membership.city}
                        </DataElement>
                    ...
                    <DataElement
                        ...
                        className="fg-0"
                        style={warnColorBecauseSepaAndActive}
                    >
                        {isActive ? <b>JA</b> : <b>NEIN</b>}
                    </DataElement>
                    <DataElement
                        ...
                        style={warnColorBecauseSepaAndActive}
                    >
                        {usesSEPA ? <b>JA</b> : <b>NEIN</b>}
                    </DataElement>
                    {(props.data.membership.useSepa ?? true) && (
                        <>
                            <Horizontal>
                                <DataElement label="Kontoinhaber/in">{props.data.membership.accountowner}</DataElement>
                                <DataElement label="Kontoinhaber/in Adresse">
                                    {props.data.membership.accountownerStreet},{props.data.membership.accountownerPlz}{' '}
                                    {props.data.membership.accountownerCity}
                                </DataElement>
                            </Horizontal>
                            <Horizontal>
                    ...
                    <DataElement label="Abholraum">{props.data.membership.abholraum}</DataElement>
